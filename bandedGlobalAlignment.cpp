...
void max(const int & s1, const int & s2, const int & s3, int & maxs,
         char & maxl) {
...
void setLR(const int & k, const int & m, const int & n, vector<int> & L,
           vector<int> & R) {
  int d = m >= n ? m - n : n - m;
  if (m >= n) {
    for (int i = 0; i <= n; i++) {
...
bool CountDiff(const char * strU, const char * strV, const int & len,
               const int & nMaxNunofErro, int & NumOfError) {
  NumOfError = 0;
  for (int i = 0; i < len; i++) {
    if (strU[i] != strV[i]) {
      NumOfError++;
