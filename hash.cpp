...
void HashToBucket(const CReference * refGenome, CHashTable * hashTable) {
	INFO("The memory of Hash Table Index Array is", sizeof(SIZE_T) * (hashTable->nSizeIndex / GB),
			"GB");
...
void ReadIndexAndRef(const Option & opt, CReference * refGenome, CHashTable * hashTable) {
	...
	FREAD_CHECK(fread(refGenome->refInBits, sizeof(InBits), refGenome->nRefSizeInWordSize, fin),
	...
	FREAD_CHECK(fread(hashTable->counter, sizeof(SIZE_T), hashTable->nSizeCounter, fin),
			hashTable->nSizeCounter);

	FREAD_CHECK(fread(&(hashTable->nSizeIndex), sizeof(SIZE_T), 1, fin), 1);
	MEMORY_ALLOCATE_CHECK(
			hashTable->index = (SIZE_T * ) malloc(sizeof(SIZE_T) * hashTable->nSizeIndex));
	FREAD_CHECK(fread(hashTable->index, sizeof(SIZE_T), hashTable->nSizeIndex, fin),
