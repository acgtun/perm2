...
void GetReadLength(Option & opt) {
  ...
  while (getline(fin, strLine)) {
    ...
    cout << "--INFO-- Full sensitivity for " << opt.mapOpt.nMaxMismatch
        << " errors." << endl;
    ...
    MEMORY_ALLOCATE_CHECK(
        opt.seedStartPos = (SIZE_T *) malloc(sizeof(SIZE_T) * opt.nSeed));
    ...
    cout << "--INFO-- The matched positions should have at least "
        << opt.mapOpt.ems << " exact seeds for each read." << endl;
    break;
  }
